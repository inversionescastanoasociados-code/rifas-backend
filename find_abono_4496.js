...
(async () => {
  ...
  try {
    const boletas = await c.query(`
      ...
      WHERE CAST(b.numero AS TEXT) = '4496'
      ORDER BY b.id
    `);
    console.log('--- BOLETAS numero 4496 ---');
    ...
    for (const b of boletas.rows) {
      const ab = await c.query(`
        SELECT a.id, a.created_at, a.monto, a.venta_id, a.boleta_id,
               mp.nombre AS medio_pago, a.gateway_pago,
      ...
      for (const vid of ventaIds) {
        const vAb = await c.query(`
          SELECT a.id, a.created_at, a.monto, a.boleta_id, mp.nombre AS medio_pago, a.estado
