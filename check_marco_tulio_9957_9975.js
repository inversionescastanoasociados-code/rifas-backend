...
(async () => {
  ...
  try {
    for (const num of ['9957', '9975']) {
      ...
      const boletas = await c.query(`
        SELECT b.id, b.numero, b.rifa_id, r.nombre AS rifa, b.estado,
               ...
               cl.nombre AS cliente_boleta, cl.identificacion AS cedula_boleta
        FROM boletas b
        JOIN rifas r ON r.id = b.rifa_id
        LEFT JOIN clientes cl ON cl.id = b.cliente_id
        WHERE CAST(b.numero AS TEXT) = $1
        ORDER BY b.id
      `, [num]);
      console.log(`--- BOLETAS numero ${num} ---`);
      console.table(boletas.rows);

      for (const b of boletas.rows) {
        console.log(`\n=== BOLETA ID=${b.id} numero=${b.numero} rifa=${b.rifa} estado=${b.estado} venta_id=${b.venta_id} ===`);

        const ventas = await c.query(`
          SELECT v.id, v.estado_venta, v.monto_total, v.abono_total,
                 v.cliente_id, cl.nombre AS cliente, cl.identificacion AS cedula,
                 cl.telefono, v.created_at, v.updated_at
          FROM ventas v
        ...
        for (const v of ventas.rows) {
          const otherB = await c.query(`
            SELECT id, numero, estado, cliente_id FROM boletas WHERE venta_id = $1 ORDER BY numero
          `, [v.id]);
          ...
          const allAb = await c.query(`
            SELECT a.id, a.created_at, a.monto, a.boleta_id, b2.numero AS boleta_num,
                   mp.nombre AS medio_pago, a.gateway_pago, a.estado
            FROM abonos a
    ...
    // Buscar adicionalmente clientes "Marco Tulio"
    console.log(`\n\n#################### CLIENTES Marco Tulio ####################`);
    const cli = await c.query(`
      SELECT id, nombre, identificacion, telefono FROM clientes
      WHERE nombre ILIKE '%marco%tulio%'
      ORDER BY nombre
    `);
